...
router.get('/dungeon1', function(req, res, next) {
    pool.getConnection(function(err, conn){
        conn.query(`SELECT * FROM monster WHERE name='슬라임';`, function(err, monster) {
            conn.query(`SELECT * FROM rpggame WHERE player_id='${req.session.ID}';`, function(err, results) {
...
router.get('/RPGgame', function(req, res, next) {
    pool.getConnection(function(err, conn){
        conn.query(`SELECT tutorial FROM rpggame WHERE player_id='${req.session.ID}';`, function(err, results) {
            if(results.length == 0) {
                conn.query(`INSERT INTO rpggame(player_id, tutorial) VALUES ('${req.session.ID}', '1');`, function(err, results) {
    ...
    pool.getConnection(function(err, conn){
        conn.query(`SELECT * FROM rpggame WHERE player_id='${req.session.ID}';`, function(err, results) {
            if(results[0].tutorial == 1) {
                conn.query(`UPDATE rpggame SET lv = 1 , nickname = '${req.body.name}', money = 1000, hp = 100, hpmax = 100, mp = 100, mpmax = 100, ad = 3, tutorial = 0 WHERE player_id='${req.session.ID}';`, function(err, results) {
                    conn.query(`SELECT * FROM rpggame WHERE player_id='${req.session.ID}';`, function(err, results) {
                        res.render('game/rpggame', {results: results});
                    });
                });
    ...
    pool.getConnection(function(err, conn){
        ...
        conn.query(`SELECT * FROM game WHERE player_id='${req.session.ID}';`, function(err, results) {
            if(results[0].money < req.body.money || req.body.money==0) {
                ...
                conn.query(`UPDATE game SET money = ${money} WHERE player_id = '${req.session.ID}';`);
                conn.query(`SELECT * FROM game WHERE player_id='${req.session.ID}';`, function(err, results) {
                    if(oddeven[0] == req.body.odd || oddeven[0] == req.body.even) {
                    ...
                    } else if(oddeven1%2==0 && oddeven[0]%2==0 || oddeven1%2==1 && oddeven[0]%2==1){
                        let money = req.body.money;
                        let money2 = req.body.money;
                        money = money * 2;
                        money2 = money2 * 2;
                        ...
                        let money2 = 0;
                        conn.query(`UPDATE game SET money = ${results[0].money} WHERE player_id = '${req.session.ID}';`, function(err, results) {
                            res.render('game/oddeven1', {results: req.body, oddeven: oddeven, money: money2});
                        });
                    }
                });
